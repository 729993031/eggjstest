...
class GetCateList extends Controller {
  async index() {
    ...
    const result = await service.requestCache.judge(
      "http://m.you.163.com/item/cateList?categoryId=1005000",
      {
        headers: {
          Accept:
    ...
    /*
         ...
         */
    const gapLength =
      result.data.categoryL1List.length - result.global.cateList.length;
    const oldList = result.global.cateList.slice();
    const newList = result.data.categoryL1List.slice(0, gapLength);
    oldList.map(
      (value, index) =>
        (value.wapBannerUrl =
          result.data.categoryL1List[index + gapLength].wapBannerUrl)
    );
